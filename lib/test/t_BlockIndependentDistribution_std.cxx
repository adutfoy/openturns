...
int main(int, char *[])
{
  ...
  try
  {
    ...
    collection[0] = ComposedDistribution(Collection<Distribution>(2, Exponential()), FrankCopula(3.0));
    collection[1] = ComposedDistribution(Collection<Distribution>(3, Gumbel()), NormalCopula(R));
    collection[2] = ComposedDistribution(Collection<Distribution>(3, Triangular()), NormalCopula(R));
    ...
    // Define a point
    Point point( distribution.getDimension(), 0.6 );
    ...
    fullprint << "isoprobabilistic transformation (general normal)=" << distribution.getIsoProbabilisticTransformation() << std::endl;
    // General case with non-normal standard distribution
    collection[0] = Student(3.0, Point(2, 1.0), Point(2, 3.0), CorrelationMatrix(2));
    distribution = BlockIndependentDistribution(collection);
    fullprint << "isoprobabilistic transformation (general non-normal)=" << distribution.getIsoProbabilisticTransformation() << std::endl;
    ...
    distribution = BlockIndependentDistribution(collection);
    fullprint << "isoprobabilistic transformation (independent)=" << distribution.getIsoProbabilisticTransformation() << std::endl;
    {
      UnsignedInteger dim = distribution.getDimension();
      Scalar x = 0.6;
      Point y(dim - 1, 0.2);
    ...
    distribution = BlockIndependentDistribution(collection);
    fullprint << "isoprobabilistic transformation (single contributor)=" << distribution.getIsoProbabilisticTransformation() << std::endl;
    {
      UnsignedInteger dim = distribution.getDimension();
      Scalar x = 0.6;
      Point y(dim - 1, 0.2);

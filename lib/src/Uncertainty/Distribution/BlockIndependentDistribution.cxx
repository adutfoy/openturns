...
void BlockIndependentDistribution::setDistributionCollection(const DistributionCollection & coll)
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    ...
    isIndependent_ = isIndependent_ && distributionCollection_[i].hasIndependentCopula();
...
Point BlockIndependentDistribution::computeDDF(const Point & point) const
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    ...
    const Point realization(distributionCollection_[i].computeDDF(pointMarg));
    std::copy(realization.begin(), realization.end(), DDF.begin()+shift);
    shift += distributionDimension;
  }
  return DDF;
}
...
Scalar BlockIndependentDistribution::computePDF(const Point & point) const
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    ...
    const Interval rangeMarg(distributionCollection_[i].getRange());
    if (!rangeMarg.contains(pointMarg)) return 0.0;
    productPDF *= distributionCollection_[i].computePDF(pointMarg);
    shift += distributionDimension;
  }
  return productPDF;
}
...
Scalar BlockIndependentDistribution::computeLogPDF(const Point & point) const
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    ...
    // If one component is outside of the support, the PDF is null
    const Interval rangeMarg(distributionCollection_[i].getRange());
    if (!rangeMarg.contains(pointMarg)) return -SpecFunc::LogMaxScalar;
    sumLogPDF += distributionCollection_[i].computeLogPDF(pointMarg);
    shift += distributionDimension;
  }
  return sumLogPDF;
}
...
Scalar BlockIndependentDistribution::computeSurvivalFunction(const Point & point) const
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    ...
    Point pointMarg(distributionDimension);
    std::copy(point.begin() + shift, point.begin() + shift + distributionDimension, pointMarg.begin());
    productSurvival *= distributionCollection_[i].computeSurvivalFunction(pointMarg);
...
BlockIndependentDistribution::PointWithDescriptionCollection BlockIndependentDistribution::getParametersCollection() const
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    ...
    const PointWithDescription distributionParameters(distributionCollection_[i].getParametersCollection()[0]);
    const Description parametersDescription(distributionParameters.getDescription());
    const String distributionName(distributionCollection_[i].getName());
    const UnsignedInteger parameterDimension = distributionParameters.getDimension();
    for (UnsignedInteger j = 0; j < parameterDimension; ++j)
    {
      point.add(distributionParameters[j]);
      description.add(OSS() << "distribution_" << i << "_" << parametersDescription[j]);
    }
  }
...
void BlockIndependentDistribution::setParametersCollection(const PointCollection & parametersCollection)
{
  ...
  for (UnsignedInteger i = 0; i < size; ++i)
  {
    // All distributions, including copulas, must output a collection of Point of size at least 1,
    // even if the Point are empty
    const UnsignedInteger atomParametersDimension = distributionCollection_[i].getParametersCollection()[0].getDimension();
    // ith distribution parameters
    ...
    distributionCollection_[i].setParametersCollection(PointCollection(1, point));
  } // atoms
